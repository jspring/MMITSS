...
int main()
{
    ...
    while(1) // main server loop
    {
        ...
        tempset = savedset;
        numready = select(maxfd+1, &tempset, &tempset, NULL, NULL); // pass max descriptor and wait indefinitely until data arrives
        ...
        if(FD_ISSET(listen_desc, &tempset)) // new client connection
        {
            ...
            printf("new client connection new fd %d old fd %d\n", conn_desc, listen_desc);
	    numclients++;
...
//                    break;
        ...
        for(k=0; k<numclients; k++) // check all clients if any received data
        {
	    printf("k %d maxi %d numready %d\n", k, maxi, numready);
            if(client[k] > 0)
            {
                if(FD_ISSET(client[k], &savedset))
                {
                    ...
                    {
                        buff[num_bytes] = '\0';
                        printf("Received from fd %d:- %s", client[k], buff);
                    }
